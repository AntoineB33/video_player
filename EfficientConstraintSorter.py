...
class EfficientConstraintSorter:
    ...
    def _solve_lexicographic_ortools(self, base_model, position, time_limit_seconds):
        ...
        for j in range(k):
            ...
            if status != cp_model.OPTIMAL and status != cp_model.FEASIBLE:
                if status == cp_model.INFEASIBLE:
                    ...
                    # Save the best solution we have so far
                    if best_solution is not None:
                        print(f"Saving best solution achieved (optimized {j}/{k} distances)...")
                        self._save_incremental_solution(best_solution, j, k, optimal_values, results_dir, timestamp, "INFEASIBLE")
                    ...
                    if best_solution is not None:
                        print(f"Solver failed with status {solver.StatusName(status)}. Saving best solution achieved...")
                        self._save_incremental_solution(best_solution, j, k, optimal_values, results_dir, timestamp, solver.StatusName(status))
            ...
            self._save_incremental_solution(current_solution, j+1, k, optimal_values, results_dir, timestamp, "ONGOING")
        ...
        # Save final complete solution
        if best_solution is not None:
            self._save_incremental_solution(best_solution, k, k, optimal_values, results_dir, timestamp, "COMPLETE")
            print(f"Final complete solution saved to: {results_dir}/solution_step_{k}_{timestamp}.txt")
        
        return solution

    def _save_incremental_solution(self, solution, step, total_steps, optimal_values, results_dir, timestamp, status):
        ...
        filename = f"solution_step_{step}_{timestamp}.txt"
        filepath = os.path.join(results_dir, filename)
        
        # Calculate actual distances for this solution
        if self.maximize_distance:
            valid_pairs = [(x, y) for x, y in self.maximize_distance if x in self.elements and y in self.elements]
            actual_distances = [abs(solution.index(x) - solution.index(y)) for x, y in valid_pairs]
            sorted_distances = sorted(actual_distances)
        else:
            actual_distances = []
            sorted_distances = []
        ...
        metadata = {
            ...
            "status": status,
            "optimized_distances": optimal_values,
            "actual_distances": actual_distances,
            "sorted_distances": sorted_distances,
            "num_elements": len(solution),
            "num_distance_pairs": len(self.maximize_distance),
            "completion_percentage": round((step / total_steps) * 100, 2) if total_steps > 0 else 0
        }
        
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write("=== INCREMENTAL OPTIMIZATION SOLUTION ===\n")
                f.write(f"Generated: {metadata['timestamp']}\n")
                f.write(f"Step: {step}/{total_steps} ({metadata['completion_percentage']}%)\n")
                f.write(f"Status: {status}\n")
                f.write(f"Optimized distances so far: {optimal_values}\n")
                f.write(f"Sorted actual distances: {sorted_distances[:10]}{'...' if len(sorted_distances) > 10 else ''}\n")
                f.write("\n=== SOLUTION ===\n")
            ...
            print(f"  Solution saved: {filename} ({metadata['completion_percentage']}% complete)")
            
        except Exception as e:
            print(f"  Warning: Could not save solution to {filepath}: {e}")
...
def print_solution(solution, errors, table_original, to_old_indexes, alph, cat_rows, attributes_table, table, roles, dep_pattern):
    if not solution:
