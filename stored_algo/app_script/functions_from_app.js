const PATTERN_DISTANCE =
  /^(?<prefix>as far as possible from )(?<any>any)?((?<number>\d+)|(?<name>.+))(?<suffix>)$/;
const PATTERN_AREAS =
  /^(?<prefix>.*\|)(?<any>any)?((?<number>\d+)|(?<name>.+))(?<suffix>\|.*)$/;
...
function getCategories(table, result, musics, music_col, roles, name) {
  ...
  for (let i = 0; i < table.length - 1; i++) {
    ...
    if (cell) {
      if (!/^(https?:\/\/|file:\/\/)/.test(cell)) {
        warnings.push(
          `Warning in row ${i + 1}, column ${alph[pathIndex]}: ${JSON.stringify(cell)} is not a valid URL or local path`,
        );
      }
    }
  }
  ...
  for (let i = 1; i < table.length; i++) {
    const row = table[i];
    for (const name of result.row_to_names[i]) {
      ...
      if (name in names) {
        errors.push(
          `Error in row ${i}, column ${alph[nameIndex]}: name ${JSON.stringify(name)} already exists in row ${names[name]}`,
        );
      }
      names[name] = i;
    }
  ...
  for (let i = 1; i < table.length; i++) {
    ...
    if (row[pathIndex] && i in attributes) {
      for (const k of Object.keys(attributes[i])) {
        if (urls[k][0]) {
          const path1 = urls[i][1]
            .map((x) => `${table[x][0].split("; ")[0]}(${x})`)
            .join(" -> ");
          const path2 = attributes[i][k][1]
            .map((x) => `${table[x][0].split("; ")[0]}(${x})`)
            .join(" -> ");
          errors.push(
  ...
  for (const cat of Object.keys(attributes)) {
    ...
    const filtered = {};
    for (const [k, v] of Object.entries(attributes[cat])) {
      if (table[k][pathIndex]) {
        filtered[k] = v;
      }
    }
    attributes[cat] = filtered;
    if (Object.keys(attributes[cat]).length === 0) {
  ...
  categories = categories.filter((_, id) => roles[id] in [Roles.ATTRIBUTES, Roles.SPRAWL]);
  categories.forEach(col => {
    col.categories.sort((a, b) => a.rows.length - b.rows.length);
  });
  result.categories = categories;

  sprawlPairs = sprawlPairs.filter(col => col[0] in [Roles.ATTRIBUTES, Roles.SPRAWL]);
  sprawlPairs.forEach(col => {
    col.pairs.sort((a, b) => a.distance - b.distance);
  });
  result.sprawlPairs = sprawlPairs;
  ...
  if (lastElement !== null) {
    for (const i of validRowIndexes) {
      if (i !== lastElement) {
        instrTable[i].push(
          new InstrStruct(true, false, [newIndexes[lastElement]], [[1, Infinity]]),
        );
      }
    }
  }
  ...
  for (let i = 1; i < table.length; i++) {
    if (!table[i][pathIndex] && !referenced[i]) {
      continue;
    }
          ...
          if (instr) {
            ...
            if (depPattern[j].length > 1) {
              instr =
                depPattern[j][0] +
                instrSplit
                  .map((split, idx) => split + depPattern[j][idx + 1])
                  .join("");
            }
      ...
      if (typeof j === "number") {
        for (const x2 of instrTable[j]) {
          ...
          if (
            ...
            const existingIndex = instrTable[i].findIndex(
              (instr) => JSON.stringify(instr) === JSON.stringify(x),
            );
            if (instrTable[i][existingIndex].path.length === 1) {
              const pathStr = [...attributes[j][i][1], ...x.path].join(
                " -> ",
              );
              warnings.push(
                `Redundant instruction ${JSON.stringify(x)} in row ${i}, column ${alph[j]} given by ${pathStr}`,
              );
            }
          }
        }
  ...
  function hasCycle(instrTable, visited, stack, node, after = true) {
    ...
    for (const neighbor of instrTable[node]) {
      if (
        ...
        (after
          ? neighbor.intervals[0][0] !== -Infinity ||
            neighbor.intervals[0][1] !== -1
          : neighbor.intervals[neighbor.intervals.length - 1][0] !== 1 ||
            neighbor.intervals[neighbor.intervals.length - 1][1] !== Infinity)
      ) {
        continue;
      }
...
function normalize(sheet, table, result, roles) {
  ...
  if (!foundError) {
    if (music_col !== -1) {
      ...
      musics = table.slice(1).map((row) => row[music_col]);
      table = table.map((row) => [
        ...row.slice(0, music_col),
        ...row.slice(music_col + 1),
      ]);
    }
